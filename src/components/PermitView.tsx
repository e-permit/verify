...
export default function PermitView() {
  ...
  const [permit, setPermit] = useState<any>(undefined);
  useEffect(() => {
    const verify = async () => {
      const data = await verifyPermit(qrCode!);
      if (data.ok) setPermit(data?.permit);
    };
    ...
    <>
      ...
      {permit && permit != null && (
        <div className="flex flex-col h-full p-6 pb-2 ">
          ...
          <div className="mt-6 border-t border-gray-100 grow overflow-y-scroll">
